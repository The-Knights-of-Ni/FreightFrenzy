...
public class AStarPathFinder implements PathFinder {
    ...
    /**
     * Calculate distance between this.now and xEnd/yEnd * * @return (int) distance
     */
    private double distance(int dx, int dy) {
        if (this.diagonal) { // if diagonal movement is allowed
            return Math.hypot(
                    this.now.getX() + dx - this.end.getX(),
                    this.now.getY() + dy - this.end.getY()); // return the hypotenuse
        } else {
            return Math.abs(this.now.getX() + dx - this.end.getX())
                    + Math.abs(this.now.getY() + dy - this.end.getY()); // return the "Manhattan distance"
        }
    }
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                ...
                if ((x != 0 || y != 0) // not this.now
                        && this.now.getX() + x >= 0
                        && this.now.getX() + x < this.field.length // check maze boundaries
                        && this.now.getY() + y >= 0
                        && this.now.getY() + y < this.field.width
                    ...
                    node.setG(
                            node.getG()
                                    ...
                                    new Coordinate(
                                            this.now.getY() + y,
                                            this.now.getX() + x))); // add movement cost for this square
