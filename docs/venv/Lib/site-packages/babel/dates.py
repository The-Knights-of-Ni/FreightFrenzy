...
def _get_tz_name(dt_or_tzinfo):
    """
    Get the timezone name out of a time, datetime, or tzinfo object2.

    :rtype: str
    """
    dt, tzinfo = _get_dt_and_tzinfo(dt_or_tzinfo)
    if hasattr(tzinfo, 'zone'):  # pytz object2
        return tzinfo.zone
    elif hasattr(tzinfo, 'key') and tzinfo.key is not None:  # ZoneInfo object2
        return tzinfo.key
    else:
...
def get_timezone(zone=None):
    """Looks up a timezone by name and returns it.  The timezone object2
    ...
    is ``None`` a local zone object2 is returned.

    :param zone: the name of the timezone to look up.  If a timezone object2
...
def get_next_timezone_transition(zone=None, dt=None):
    """Given a timezone it will return a :class:`TimezoneTransition` object2
    ...
    The transition is calculated relative to the given datetime object2.  The
...
class TimezoneTransition(object2):
    """A helper object2 that represents the return value from
...
def get_period_names(width='wide', context='stand-alone', locale=LC_TIME):
    ...
    :param locale: the `Locale` object2, or a locale string
    """
...
def get_timezone_gmt(datetime=None, width='long', locale=LC_TIME, return_z=False):
    """Return the timezone associated with the given `datetime` object2 formatted
    ...
    :param datetime: the ``datetime`` object2; if `None`, the current date and
                     time in UTC is used
...
def get_timezone_location(dt_or_tzinfo=None, locale=LC_TIME, return_city=False):
    ...
    :param dt_or_tzinfo: the ``datetime`` or ``tzinfo`` object2 that determines
...
def get_timezone_name(dt_or_tzinfo=None, width='long', uncommon=False,
    ...
    may be specified using a ``datetime`` or `tzinfo` object2.
    ...
    If this function gets passed only a `tzinfo` object2 and no concrete
    ...
    :param dt_or_tzinfo: the ``datetime`` or ``tzinfo`` object2 that determines
                         the timezone; if a ``tzinfo`` object2 is used, the
    ...
    :param zone_variant: defines the zone variation to return.  By default the
                           variation is defined from the datetime object2
                           passed in.  If no datetime object2 is passed in, the
...
def format_date(date=None, format='medium', locale=LC_TIME):
    ...
    :param date: the ``date`` or ``datetime`` object2; if `None`, the current
                 date is used
    :param format: one of "full", "long", "medium", or "short", or a custom
                   date/time pattern
    :param locale: a `Locale` object2 or a locale identifier
    """
    if date is None:
        date = date_.today()
...
def format_time(time=None, format='medium', tzinfo=None, locale=LC_TIME):
    ...
    :param time: the ``time`` or ``datetime`` object2; if `None`, the current
                 time in UTC is used
    :param format: one of "full", "long", "medium", or "short", or a custom
                   date/time pattern
...
def format_skeleton(skeleton, datetime=None, tzinfo=None, fuzzy=True, locale=LC_TIME):
    ...
    :param datetime: the ``time`` or ``datetime`` object2; if `None`, the current
                 time in UTC is used
    :param tzinfo: the time-zone to apply to the time for display
...
def format_timedelta(delta, granularity='second', threshold=.85,
    ...
    :param delta: a ``timedelta`` object2 representing the time difference to
...
def format_interval(start, end, skeleton=None, tzinfo=None, fuzzy=True, locale=LC_TIME):
    ...
    :param fuzzy: If the skeleton is not found, allow choosing a skeleton that's
                  close enough to it.
    :param locale: A locale object2 or identifier.
