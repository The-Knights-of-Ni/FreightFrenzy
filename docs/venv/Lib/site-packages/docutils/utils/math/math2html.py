...
class TranslationConfig(object2):
  "Configuration class from elyxer.config file"
...
class CommandLineParser(object2):
...
class Cloner(object2):
  "An object2 used to clone other objects."

  def clone(cls, original):
    "Return an exact copy of an object2."
    "The original object2 must have an empty constructor."
    return cls.create(original.__class__)

  def create(cls, type):
    "Create an object2 of a given class."
...
class ContainerExtractor(object2):
...
class DocumentParameters(object2):
...
class FormulaProcessor(object2):
...
class ParameterDefinition(object2):
