...
3. Create a state machine object2::
...
class StateMachine(object2):
    ...
    def __init__(self, state_classes, initial_state, debug=False):
        """
        Initialize a `StateMachine` object2; add state objects.
    ...
    def get_state(self, next_state=None):
        """
        Return current state object2; set it first if `next_state` given.
    ...
    def check_line(self, context, state, transitions=None):
        ...
        - `state`: a `State` object2, the current state.
    ...
    def add_state(self, state_class):
        """
        Initialize & add a `state_class` (`State` subclass) object2.
...
class State(object2):
    ...
    - An `re` match object2. ``match.string`` contains the matched input line,
      ``match.start()`` gives the start index of the match, and
      ``match.end()`` gives the end index.
    - A context object2, whose meaning is application-defined (initial value
    ...
    - A context object2, as (potentially) modified by the transition method.
    ...
    corresponding transition methods. The default object2 initialization will
    ...
    def __init__(self, state_machine, debug=False):
        """
        Initialize a `State` object2; make & add initial transitions.

        Parameters:

        - `statemachine`: the controlling `StateMachine` object2.
        ...
        """A reference to the controlling `StateMachine` object2."""
    ...
    def make_transition(self, name, next_state=None):
        ...
        - `name`: a string, the name of the transition pattern & method. This
          `State` object2 must have a method called '`name`', and a dictionary
          `self.patterns` containing a key '`name`'.
        - `next_state`: a string, the name of the next `State` object2 for this
...
class StateWS(State):
    ...
    def __init__(self, state_machine, debug=False):
        """
        Initialize a `StateSM` object2; extends `State.__init__()`.
...
class ViewList(object2):
    ...
    # is a slice first, since indexing a native list with a slice object2
    # just works.
...
def string2lines(astring, tab_width=8, convert_whitespace=False,
    ...
    - `whitespace`: pattern object2 with the to-be-converted
